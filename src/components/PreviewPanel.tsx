...
export const PreviewPanel: React.FC<PreviewPanelProps> = ({ settings }) => {
  ...
  const sampleContent = Array.from({ length: 50 }, (_, i) => (
    <div key={i} className="p-4 mb-2 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-700 rounded-lg border border-gray-200 dark:border-gray-600">
      ...
      {(settings.orientation === 'vertical' || settings.orientation === 'both') && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="p-4 bg-gradient-to-r from-blue-500 to-indigo-600 text-white">
            <h3 className="text-lg font-semibold">Vertical Scrollbar Preview</h3>
            <p className="text-blue-100">Scroll vertically to see your custom scrollbar</p>
          </div>
          <div
            ref={verticalRef}
            className="custom-scrollbar h-80 overflow-y-auto p-4 bg-gray-50 dark:bg-gray-900"
          >
            {sampleContent}
          </div>
        </div>
      ...
      {(settings.orientation === 'horizontal' || settings.orientation === 'both') && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="p-4 bg-gradient-to-r from-green-500 to-teal-600 text-white">
            <h3 className="text-lg font-semibold">Horizontal Scrollbar Preview</h3>
            <p className="text-green-100">Scroll horizontally to see your custom scrollbar</p>
          </div>
          <div
            ref={horizontalRef}
            className="custom-scrollbar h-40 overflow-x-auto p-4 bg-gray-50 dark:bg-gray-900"
          >
            <div className="flex gap-4" style={{ width: '200%' }}>
              {Array.from({ length: 20 }, (_, i) => (
                <div
                  key={i}
                  className="flex-shrink-0 w-64 p-4 bg-gradient-to-br from-purple-50 to-pink-50 dark:from-gray-800 dark:to-gray-700 rounded-lg border border-gray-200 dark:border-gray-600"
                >
